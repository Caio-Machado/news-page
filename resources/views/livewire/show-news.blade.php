<div>
    ...
    <ul class="mt-8 flex flex-wrap -mx-4">
        @foreach ($articles as $article)
            <li class="mb-6 w-full md:w-1/2 lg:w-1/3 px-4">
                <div class="bg-white p-4 shadow-md rounded h-full">
                    <a href="{{ route('article.show', $article->id) }}" class="block">
                        <div class="mb-4">
                            <img src="{{ $article->getImageUrl() }}" alt="Imagem do artigo" class="w-full h-48 object-cover rounded">
                        </div>
                    ...
                    @auth
                        @if ($article->author->name == Auth::user()->name)
                            <div class="flex mt-2">
                                <a href="{{ route('edit-article', $article->id) }}" class="px-4 py-2 mr-4 text-red-500 cursor-pointer hover:text-red-900 border border-red-500 rounded-md">
                                    Editar
                                </a>
                            </div>
                        @endif
